...
/// - 'TrentinoWinter': Represents the coldest winter weather possible, comprised of snow.
/// .....
/// # Usage
///
/// ```
/// use robotics_lib::world::environmental_conditions::WeatherType;
/// let weather = WeatherType::TrentinoWinter;
///
/// match weather {
...
///     WeatherType::TrentinoWinter => println!("You need energy to heat up")
/// }
/// ```
...
impl EnvironmentalConditions {
    ...
    pub fn new(weather_forecast: &[WeatherType], time_progression_minutes: u8, starting_hour: u8) -> Self {
        if weather_forecast.is_empty() {
            panic!("The forecast cannot be empty.")
        }
        EnvironmentalConditions {
    ...
    pub fn get_time_of_day(&self) -> DayTime {
        match self.time_of_day.hour {
            ...
            | _ => panic!("The hour must be between 0 and 24"),
        }
    }
...
impl TimeOfDay {
    pub(crate) fn advance(&mut self, time_progression_minutes: u8) -> bool {
        let m = self.minute + time_progression_minutes;
        if m > 59 {
            let hours_to_add = m / 60;
            let minutes_reminder = m % 60;
            self.hour += hours_to_add;
            self.minute = minutes_reminder;
        } else {
            self.minute = m;
        }
