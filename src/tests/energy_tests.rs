...
#[test]
fn cannot_set_more_than_max() {
    let mut en = Energy::default();
    en.set_energy_level(MAX_ENERGY_LEVEL + 1);
    assert_eq!(en.get_energy_level(), MAX_ENERGY_LEVEL)
}
...
fn set_to_max() {
    let mut en = Energy::default();
    en.set_energy_level(MAX_ENERGY_LEVEL);
}
#[test]
fn set_to_random() {
    let mut en = Energy::default();
    ...
    en.set_energy_level(rnd_number);
